...
## Локальный реестр webhook

`WebhookDetails.confirmed_destinations` сохраняет совместимое имя поля,
но означает число записей реестра владения Core. Отсутствие поля или
`null` сохраняется как неизвестное; явный `0` остаётся нулём. Адаптер
не восстанавливает значение из статуса `active` или списка событий.
Количество не подтверждает ни наличие подписки в amoCRM сейчас, ни
фактическую доставку. Эти ограничения действуют и для положительного
числа: Core сохраняет желаемый адрес до внешнего вызова регистрации.
