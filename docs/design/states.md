...
| `stale` | Из кеша старше порога | Метка «данные от HH:MM», тон `attention` |
...
Реестр бекендов (`catalog.Registry.Probe`) сейчас выставляет
...
Локальные состояния сами по себе не подтверждают доступ. `unverified`
снимается только при свежей успешной проверке текущей версии credentials.
Классификация проверки передаётся отдельно (`verified_ok`, `auth_error`,
`network_error`, `rate_limited`, `internal_error`) с временем попытки.
...
Число `confirmed_destinations` — записи локального реестра владения,
не результат живой проверки подписки или доставки. Желаемый адрес может
появиться в реестре до успешного внешнего запроса. Старые регистрации
могут работать без записи в реестре до следующей сверки.

- `active` и явный ноль: «Регистрация webhook требует сверки»,
  тон `attention`; не утверждать, что события не доставляются.
- Отсутствующее/null число: «Нет данных локального реестра webhook»;
  не подставлять ноль и не предлагать восстановление по этому факту.
- Устаревшее наблюдение: «Данные о регистрации webhook устарели»;
  сначала обновить наблюдение.
- `unavailable` остаётся ошибкой источника, `error` — ошибкой регистрации.
- Проверка подключения подтверждает доступ к `/account`, но не сверяет
  webhook. Время сверки регистрации и время проверки доступа различны.
...
unknown; старше 90 минут — stale (fallback старого Core — 15 минут). Отрицательный результат проверки может
...
Переключатель «Показать пароль» / «Скрыть пароль» меняет только видимость
локального поля. Во время отправки формы кнопка «Войти» недоступна;
`role=status` сообщает «Выполняется вход…». Ошибка остаётся над полями
с `role=alert`, повторный вход доступен после завершения запроса.

- `401`: «Неверный email или пароль», одинаково для неизвестного email,
  неверного пароля и отключённого сотрудника.
- `429`: «Слишком много попыток, повторите через 60 с».
- `5xx`, сетевой сбой и прочие неожиданные ответы: «Сервис входа временно
  недоступен. Повторите попытку позже.». При наличии `request_id` показать
  код обращения; внутреннее сообщение сервера не выводить.

## Актуальность проверок (2026-09-19)

Уточняет правила аккаунта выше: `ok` допустим только когда каждая active
установка имеет `authorization_check.classification=verified_ok` и
`freshness=fresh`, допустимую локальную авторизацию и не имеет других проблем.
Unknown/stale даёт attention с отдельным verification-бейджем. Auth error
даёт needs_action; network/429/internal дают attention без reauth. Freshness
определяет Core (5400 секунд); HTTP observed_at не продлевает проверку.

| Проверка | Тон | Текст |
| --- | --- | --- |
| unknown | unknown | Доступ не проверен |
| stale | attention | Проверка устарела |
| unavailable источника | error | Источник недоступен |

Общий блок называется «Состояние установки». «Локальная авторизация»
