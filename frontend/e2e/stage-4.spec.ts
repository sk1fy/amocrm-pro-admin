...
test('registry lists both backends with names, state, contract and capabilities', async ({
  ...
  const table = page.getByRole('table', { name: 'Реестр бекендов' })
  ...
  await expect(coreRow).toContainText('Fixture Core')
  await expect(moduleRow).toContainText('Fixture module (тестовые данные)')
  await expect(coreRow.getByTestId('backend-status-core')).toHaveText('Доступен')
  await expect(moduleRow.getByTestId('backend-status-fixture')).toHaveText('Доступен')

  const coreCells = coreRow.locator('td')
  const moduleCells = moduleRow.locator('td')
  await expect(coreCells.nth(0)).toHaveText('fixture')
  await expect(moduleCells.nth(0)).toHaveText('fixture')
  await expect(coreCells.nth(2)).toHaveText('v1')
  await expect(moduleCells.nth(2)).toHaveText('v1')
  await expect(coreCells.nth(4)).toContainText('subscriptions')
  await expect(moduleCells.nth(4)).not.toContainText('subscriptions')
  await expect(coreCells.nth(5)).toContainText('lead-status')
  await expect(coreCells.nth(5)).toContainText('activity')
  await expect(moduleCells.nth(5)).toContainText('fixture-module')
  await expect(coreCells.nth(6)).toContainText('accounts')
  await expect(coreCells.nth(7)).toHaveText('—')
  await expect(coreCells.nth(8)).toContainText(/\d{1,2}:\d{2}/)
  await expect(moduleCells.nth(8)).toContainText(/\d{1,2}:\d{2}/)
})
...
test('roles, session revocation and audit end to end', async ({ page, browser }) => {
  ...
  await expect(viewerPage.getByRole('table', { name: 'Реестр бекендов' })).toBeVisible()
  ...
  const createEntry = page.getByRole('row').filter({ hasText: viewerId }).first()
  ...
  const revokeEntry = page.getByRole('row').filter({ hasText: viewerId }).first()
